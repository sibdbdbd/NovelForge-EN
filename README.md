...
## 📅 Changelog
<details open>
<summary>Unreleased — validation hardening for long-form serials</summary>

- **Spoiler detection is proposition-level** (`app/services/forge/spoilers.py`, shared by `validate_outline`, `validate_pov` and the Continuity Guard): a prohibited statement is decomposed into the names it is about and the specific terms it asserts, and a sentence matches only when it reproduces that proposition (inflection-tolerant, never prefix-based, questions and negated identities excluded). Two shared words in a two-sentence window are no longer a critical `future_beat_advanced` / `forbidden_reveal`. Ambiguous overlaps are advisory (`medium`); high-confidence reproductions block. Linear scan: ~2,000 statements against a chapter in tens of milliseconds instead of ~15 s.
- **Compiler future window is bounded**: forbidden outcomes derived from upcoming outlines cover only the next `FUTURE_OUTLINE_WINDOW` (5) chapters, capped at `FUTURE_OUTLINE_MAX_ITEMS` (24), and skip anything the current chapter also plans. Knowledge boundaries remain the job of explicit Knowledge Fact cards. Chapter-numbered card lookups are indexed once per compile.
- **Entity recognition** (`claims.named_entities`) requires positional evidence: a capitalised word is a name only when it appears mid-sentence, as a possessive, as part of a multi-word name, or recurs at sentence starts without being an ordinary word (shared `lexicon`: stop words, numbers, titles, colours, inflected verbs, adverbs, genre nouns). "Twelve", "Opened", "Counting", "Kept", "Adjusted", "Suddenly" are never characters; "Signet Brooch" is one entity. An unplanned name blocks (`unauthorized_entity`, high) only from **3** mentions; 1–2 mentions are advisory.
- **Firewall**: contractions ("I've") and ordinary capitalised words can no longer become source-distinctive terms; a single distinctive-term hit is `medium`, a cluster of ≥3 is `high`. Source entity names, phrase copies, dialogue copies and quotation runs still block. The `SourceProfile` is cached per source content instead of being rebuilt for every chapter.
- **Severity**: style-adherence findings are advisory only (`medium`/`low`); canon contradictions (`unplanned_death`, `relationship_reset`, `duplicate_payoff`, …) still block.
- **Repair loop** stops when it is futile — the editor returns an empty/unchanged draft or exactly the same blocking findings survive a targeted rewrite — records `repair_stalled` in the run report and never ends on a rewrite that validated worse than an earlier one.
- **Chapter planner** prompt keeps full detail for the last 12 committed chapters and one compressed line for older ones (capped), so 400-chapter serials cannot overflow the planning context.
- Tests: `backend/tests/test_forge_false_positives.py` (regression suite for the traps above), updates to `test_forge_units.py` / `test_forge_pipeline.py`.
